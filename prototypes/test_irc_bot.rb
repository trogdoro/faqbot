...
class IRC
  ...
  def handle_server_input(s)
    ...
    when /PRIVMSG #crbcodejam :(.+)/
      msg = $1
      puts "[ CHANNEL MSG: #{msg} ]"

      if(msg =~ /^fqbot:\s*(.+)$/)
        open("http://summize.com/search.json?q=#{CGI.escape($1)}") do |f|
          first_line = f.readline
          puts "DEBUG: first_line length: #{first_line.length}"

          json = JSON.parse(first_line)
          results = json["results"]
          puts "DEBUG: Num of Results: #{results.size}"

          if results.size > 0
            send_to_channel '#crbcodejam', "#{results.first['from_user']} - #{results.first['text']}"
          end
        end
      end
    else
...
irc = IRC.new('irc.freenode.net', 6667, 'fqbot', '#crbcodejam')
irc.connect()
begin
  irc.main_loop()
rescue Interrupt
